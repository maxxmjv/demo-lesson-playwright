...
export class OrderPage extends BasePage {
  ...
  async checkInnerComponents(): Promise<void> {
    await this.statusButton.checkButtonVisible(true)
    await expect(this.title).toBeVisible({ timeout: 15000 })
    await this.createOrderButton.checkButtonVisible(true)
  ...
  async checkCreateOrderBtnEnabled(enabled: boolean): Promise<void> {
    await this.createOrderButton.checkButtonEnabled(enabled)
  }
